...
if p.Hostname == "" { p.Hostname = safeHost(p.Name) }
...
defer rows.Close()
m := map[int]bool{}
for rows.Next() { var p int; if err := rows.Scan(&p); err != nil { return nil, err }; m[p] = true }
return m, rows.Err()
}
...
var rev []app.LogLine
for rows.Next() {
var l app.LogLine; var ts string
if err := rows.Scan(&l.ProjectID, &l.Stream, &l.Line, &ts); err != nil { return nil, err }
...
var p app.Project; var auto int; var last, updated string
...
for _, r := range name {
ok := (r >= 'a' && r <= 'z') || (r >= '0' && r <= '9')
if ok { b.WriteRune(r); lastDash = false; continue }
if !lastDash { b.WriteByte('-'); lastDash = true }
}
out := strings.Trim(b.String(), "-")
if out == "" { return fmt.Sprintf("project-%d", time.Now().Unix()) }
return out
}
